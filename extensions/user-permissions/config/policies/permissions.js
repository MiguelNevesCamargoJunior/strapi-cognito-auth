...
module.exports = async (ctx, next) => {
  ...
  if (ctx.request && ctx.request.header && ctx.request.header.authorization) {
    try {
      ...
      //TOKEN NAO VALIDADO NOS PROVIDERS DO STRAPI - TESTAR O ACCESS TOKEN NO COGNITO
      ...
      try {
        const data = await axios({
          ...
          data: {
            'AccessToken': ctx.request.header.authorization.split(' ')[1]
          },
          headers: {
        ...
        //SE ACCESS TOKEN É VALIDO, AUTENTICA
        return await next();
      } catch (error) {

        //ACCESS TOKEN INVÁLIDO - ACESSO NEGADO
        console.log("ERROR", error);
        return handleErrors(ctx, new Error('Invalid token: Token did not match with Strapi or Cognito'), 'unauthorized');
      }
    }
